...
#Delete Trail ID - Works as intended
def delete_trail_id(trail_id):
    existing_trail_id = Trail_Feature.query.filter(Trail_Feature.trail_id == trail_id).all()

    if existing_trail_id:
        for x in existing_trail_id:
            db.session.delete(x)
        db.session.commit()
        return make_response(f'{existing_trail_id} has been successfully deleted', 200)
    else:
        abort(404, f'Trail with trail ID {trail_id} cannot be found')


#Delete Trail Feature ID - Works as intended
def delete_trail_feature_id(trail_feature_id):
    existing_trail_feature_id = Trail_Feature.query.filter(Trail_Feature.trail_feature_id == trail_feature_id).all()

    if existing_trail_feature_id:
        for x in existing_trail_feature_id:
            db.session.delete(x)
        db.session.commit()
        return make_response(f'{existing_trail_feature_id} has been successfully deleted', 200)
    else:
        abort(404, f'Trail feature with trail feature ID {trail_feature_id} cannot be found')
